...
git clone https://github.com/sandraschi/sandraschi
cd sandraschi
just
```
...
4. Start the server:
   ...
   uv run python -m sandraschi.server

   # HTTP mode (for web dashboard)
   uv run uvicorn sandraschi.server:app --port 
   ```
...
| Something else | [Open a GitHub issue](https://github.com/sandraschi/sandraschi/issues) |
